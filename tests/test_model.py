...
from mixsiapy.model import build_model, run_model
import numpy as np
import pytest
...
def test_fixed_effect_uses_reference_level():
    from pathlib import Path
    from mixsiapy import load_mix_data, load_source_data, load_discr_data

    data = Path(__file__).parents[1] / "mixsiapy" / "data"
    mix = load_mix_data(
...
def test_combine_sources_and_continuous_prediction():
    import arviz as az
    import numpy as np
    from mixsiapy import combine_sources, continuous_effect_prediction
...
def test_effect_input_validation():
    from pathlib import Path
    from mixsiapy import load_mix_data

    data = Path(__file__).parents[1] / "mixsiapy" / "data" / "alligator_consumer.csv"
...
def test_invalid_error_combinations_and_single_observation(tmp_path):
    ...
    one = pd.read_csv(
        Path(__file__).parents[1] / "mixsiapy" / "data" / "alligator_consumer.csv"
    ).iloc[:1]
