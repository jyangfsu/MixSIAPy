...
DEFAULT_OUT = ROOT / "benchmarks" / "numerical_agreement_v2"
...
SMOKE = dict(chains=2, chain_length=1000, burn=500, thin=5,
             ...
             py_target_accept=.90, py_max_treedepth=10)
...
parser.add_argument("--engines", nargs="+", choices=("r", "python"),
                    default=("r", "python"))
    ...
    for engine in args.engines:
        for round_name in (["smoke"] if args.smoke else args.rounds):
            ...
            if done.exists() and not args.force:
                ...
                if engine == "r":
                    cmd = [str(R_EXE), str(ROOT / "benchmarks" / "numerical_runner_r.R"),
                    ...
                    cmd = [sys.executable, str(ROOT / "benchmarks" / "numerical_runner_python.py"),
                           ...
                           "--target-accept", str(sampling["py_target_accept"]),
                           "--max-treedepth", str(sampling["py_max_treedepth"])]
                if args.force:
                    cmd.append("--force")
                ...
                if returncode:
                    break
            # Both statuses require a longer run. NEEDS_LONGER_RUN indicates
            # borderline diagnostics, whereas NOT_CONVERGED indicates a clear
            # failure of one or more convergence thresholds.
            if args.smoke or metadata.get("convergence_status") not in {
                "NEEDS_LONGER_RUN", "NOT_CONVERGED"
            }:
                break
