"""Run one checkpointed MixSIAPy/PyMC CPU numerical-agreement fit."""
...
from mixsiapy import load_discr_data, load_mix_data, load_source_data, run_model
...
parser.add_argument("--output-root", type=Path,
                    default=ROOT / "benchmarks" / "numerical_agreement")
...
try:
    ...
    fit = run_model(
        ...
        nuts={"max_treedepth": args.max_treedepth}, progressbar=True,
    )
    ...
    # Numerical-agreement rule used in the manuscript. MCSE is retained in
    # the output for interpretation, but it is not a hard pass/fail criterion.
    strict = (max_rhat <= 1.05 and min_bulk >= 100 and min_tail >= 100
              and divergences == 0
              and treedepth_hits == 0 and min_bfmi > 0.30)
    longer = (not strict and max_rhat < 1.10 and min_bulk >= 50
              and min_tail >= 50 and divergences == 0
              and treedepth_hits == 0 and min_bfmi > 0.30)
    convergence_status = (
    ...
    metadata = {
        ...
        "module_path": str(Path(sys.modules["mixsiapy"].__file__).resolve()),
        ...
        "convergence_rule": (
            ...
            "zero divergences/treedepth hits; BFMI > 0.30"
        ),
    }
