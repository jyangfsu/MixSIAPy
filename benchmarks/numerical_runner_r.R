...
data_dir <- file.path(root, "mixsiapy", "data")
...
tryCatch({
  ...
  # Numerical-agreement benchmark rule, aligned with the conventional
  # MixSIAR Gelman-Rubin threshold. ESS guards against effectively stuck
  # chains even when R-hat alone appears acceptable.
  strict <- max_rhat < 1.05 && min_bulk >= 100 && min_tail >= 100
  longer <- !strict && max_rhat < 1.10 && min_bulk >= 50 && min_tail >= 50
