...
def build_model(mix, source, discr, alpha_prior=1, process_err=True, resid_err=True,
    ...
    try:
        ...
        raise ImportError(
            "Install mixsiapy[bayes] to build and sample models"
        ) from exc
    ...
    with pm.Model(coords=coords) as model:
        ...
        for idx, fac in enumerate(mix["FAC"]):
            if fac["re"]:
                ...
                if noncentered:
                    # z ~ N(0,1), effect = sigma*z preserves N(0,sigma).
                    z = pm.Normal(f"fac{idx+1}_z", 0, 1,
                                  dims=(f"fac{idx+1}_level", "ilr"))
                    effect = pm.Deterministic(f"ilr_fac{idx+1}", sigma * z,
                                              dims=(f"fac{idx+1}_level", "ilr"))
                else:
                    effect = pm.Normal(
