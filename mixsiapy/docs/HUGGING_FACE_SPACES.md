# Deploying MixSIAPy on Hugging Face Spaces

MixSIAPy must use a **Docker Space**, not a Static Space. The GUI is a
...
The root `Dockerfile` installs `mixsiapy[bayes,gui]` and starts Streamlit on
that port.
...
```powershell
docker build -t mixsiapy-space .
docker run --rm -p 7860:7860 mixsiapy-space
```
...
Face token with write access to `jyang-research/MixSIAPy`. Every push to the
