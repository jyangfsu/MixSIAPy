...
MixSIAPy can run the same PyMC probability model with NumPyro or BlackJAX
...
Installing `mixsiapy[jax]` installs the Python-side sampler dependencies, but
...
```text
python -m mixsiapy.examples.wolves_gpu --run test
```
