# R MixSIAR to MixSIAPy model map
...
- R reports DIC. MixSIAPy uses ArviZ PSIS-LOO and WAIC and does not currently reproduce DIC.
...
- MixSIAPy stores additional deterministic variables and labeled xarray dimensions. Extra variables do not alter the likelihood.
- For two fixed effects or one fixed plus one random effect, R constructs `p.both` during output processing and only for observed factor combinations. MixSIAPy calculates the full Cartesian array in the model; downstream comparisons must mask combinations absent from the mixture data.
