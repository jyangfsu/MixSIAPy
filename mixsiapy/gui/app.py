"""Interactive, code-free MixSIAPy workflow."""
...
from mixsiapy import (
    backend_status, build_model, get_resource_path, load_discr_data,
    load_mix_data, load_source_data, run_model, save_results,
)
from mixsiapy.backends import resolve_backend
from mixsiapy.gui import write_reproducibility_bundle
from mixsiapy.plotting import plot_data, plot_prior
from mixsiapy.model import RUN_PRESETS


ASSET_DIR = Path(__file__).resolve().parent / "assets"
st.set_page_config(
    page_title="MixSIAPy · Bayesian Tracer Mixing",
...
import base64
_logo64 = base64.b64encode((ASSET_DIR / "mixsiapy-logo.png").read_bytes()).decode()
st.markdown(f"""
<div class="hero">
  <img src="data:image/png;base64,{_logo64}" alt="MixSIAPy logo"/>
  <div><div class="eyebrow">Open Bayesian mixing platform</div>
  <h1>MixSIA<span>Py</span></h1>
...
EXAMPLES = {
    "Crop water uptake (generalized regression)": {
        ...
        "source_type": "raw",
        "formula": "species * biomass + species * diversity",
        "continuous": [],
        "factors": ["spcomp"], "random": [True], "nested": [False],
        "source_factor": None,
...
with st.sidebar:
    st.markdown(f'<div class="sidebar-brand"><img src="data:image/png;base64,{_logo64}"/><div><b>MixSIA<span>Py</span></b><br><small>Analysis workspace</small></div></div>', unsafe_allow_html=True)
    st.markdown("#### Project")
...
with tab_model:
    ...
    formula = st.text_input(
        ...
        key="gui_formula",
        help="Examples: depth + temperature; species * biomass + species * diversity. '*' includes main effects and interaction.",
    ).strip()
    if formula:
    ...
    work = Path(tempfile.mkdtemp(prefix="mixsiapy_gui_validate_"))
    try:
...
with tab_run:
    ...
    if run_clicked:
        result_dir = Path(tempfile.mkdtemp(prefix="mixsiapy_gui_result_"))
...
""")
        ...
        st.download_button(
            ...
            zip_directory(result_dir), "mixsiapy_analysis.zip", "application/zip",
            type="primary",
        )

st.divider()
st.caption("MixSIAPy research preview · GUI and Python API use the same model implementation")
